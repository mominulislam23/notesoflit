 ...
 * @return void
 */
if( ! function_exists( 'neville_breadcrumbs_trail' ) ) {
	function neville_breadcrumbs_trail( $args = array() ) {

		$breadcrumb = apply_filters( 'neville_breadcrumb_trail___object', null, $args );

		if ( ! is_object( $breadcrumb ) )
			$breadcrumb = new Neville_Breadcrumb_Trail( $args );

		return $breadcrumb->trail();
	}
}

add_filter( 'neville_breadcrumb_trail___labels', function( $defaults ) {
	$new = array(
		'home' => '<i class="nicon nicon-home-c"></i>',
	);
 ...
 * @access public
 */
if( ! class_exists( 'Neville_Breadcrumb_Trail' ) ) {
	class Neville_Breadcrumb_Trail {
		...
		public function __construct( $args = array() ) {
			...
			$this->args = apply_filters( 'neville_breadcrumb_trail___args', wp_parse_args( $args, $defaults ) );
		...
		public function trail() {
			...
			if ( 0 < $item_count ) {
				...
				foreach ( $this->items as $item ) {
					...
					// Build the list item.
					$breadcrumb .= sprintf( '<li %s>%s%s <i class="nicon nicon-angle-right"></i></li>', $attributes, $item, $meta ); // MADE CHANGES
				}
			...
			$breadcrumb = apply_filters( 'neville_breadcrumb___trail', $breadcrumb, $this->args );
		...
		protected function set_labels() {
			...
			$defaults = array(
				'browse'              => esc_html__( 'Browse:',                               'neville' ),
				'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'neville' ),
				'home'                => esc_html__( 'Home',                                  'neville' ),
				'error_404'           => esc_html__( '404 Not Found',                         'neville' ),
				'archives'            => esc_html__( 'Archives',                              'neville' ),
				// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
				'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'neville' ),
				// Translators: %s is the page number.
				'paged'               => esc_html__( 'Page %s',                               'neville' ),
				// Translators: Minute archive title. %s is the minute time format.
				'archive_minute'      => esc_html__( 'Minute %s',                             'neville' ),
				// Translators: Weekly archive title. %s is the week date format.
				'archive_week'        => esc_html__( 'Week %s',                               'neville' ),
			...
			$this->labels = apply_filters( 'neville_breadcrumb_trail___labels', wp_parse_args( $this->args['labels'], $defaults ) );
		}
		...
		protected function set_post_taxonomy() {
			...
			$this->post_taxonomy = apply_filters( 'neville_breadcrumb_trail___post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
		}
		...
		protected function add_items() {
			...
			$this->items = array_unique( apply_filters( 'neville_breadcrumb_trail___items', $this->items, $this->args ) );
		}
		...
		protected function add_minute_hour_archive_items() {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'neville' ) ) );
		}
		...
		protected function add_minute_archive_items() {
			...
			// Add the minute item.
			if ( true === $this->args['show_title'] )
				$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'neville' ) ) );
		}
		...
		protected function add_hour_archive_items() {
			...
			// Add the hour item.
			if ( true === $this->args['show_title'] )
				$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'neville' ) ) );
		}
		...
		protected function add_day_archive_items() {
			...
			$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'neville' ) ) );
			$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'neville' ) ) );
			$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'neville' ) ) );
		...
		protected function add_week_archive_items() {
			...
			$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'neville' ) ) );
			$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'neville' ) ) );
		...
		protected function map_rewrite_tags( $post_id, $path ) {
			...
			if ( is_array( $matches ) ) {
				...
				foreach ( $matches as $match ) {
					...
					if ( '%year%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'neville' ) ) ) );

					// If using the %monthnum% tag, add a link to the monthly archive.
					elseif ( '%monthnum%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'neville' ) ) ) );

					// If using the %day% tag, add a link to the daily archive.
					elseif ( '%day%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'neville' ) ) ) );
